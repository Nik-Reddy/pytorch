...
def main() -> None:
    ...
    if options.headeronly_install_dir:
        headeronly_install_dir = options.headeronly_install_dir
    else:
        # Default: derive from install_dir (build/aten/src/ATen -> build/torch/headeronly/core)
        headeronly_install_dir = str(
            Path(options.install_dir).parent.parent.parent
            / "torch"
            / "headeronly"
            / "core"
        )
